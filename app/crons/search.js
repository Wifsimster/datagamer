        ...
        new CronJob('/45 ' + search_cron + ' * * *', function () {
            ...
            request('http://localhost:' + config.general.port + '/wanted/games', function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    recursiveSearch(0, config, result.games, function () {
                        winston.info('CRON Search - Search ended !');
                    });
                } else {
...
// Recursive search
function recursiveSearch(i, config, games, callback) {
    if (i < games.length) {
                                ...
                                if (result.code == 201) {
                                    ...
                                    wanted_db.update({name: game.name}, game, function (err, newDoc) {
                                        if (!err) {
                                            ...
                                            recursiveSearch(i + 1, config, games, callback);
                                        } else {
                                            winston.error("CRON Search - " + error);
                                            callback();
                                        }
